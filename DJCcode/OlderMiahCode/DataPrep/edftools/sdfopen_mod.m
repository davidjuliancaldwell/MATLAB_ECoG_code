function [EDF,H1,h2]=sdfopen_mod(arg1,arg2,arg3,arg4,arg5,arg6)
	...
	EDF.SIE.RR=1;
        ...
        if EDF.SIE.TECG == 1; 
	        ...
	        if ~isempty(Index)
                ...
                for ki=1:length(EDF.SIE.InChanSelect),
                        k=find(OutChanSelect==EDF.SIE.InChanSelect(ki));
                        if isempty(k)
                                EDF.TECG.QRStemp(:,ki) = zeros(fs,1);
                        else
                                EDF.TECG.QRStemp(:,ki) = QRS.Templates(0.5*fs:1.5*fs-1,k).*hanning(fs);
                        end;
                        [tmp,EDF.TECG.Z(:,ki)] = filter(EDF.TECG.QRStemp(:,ki),1,pulse);
                end;
