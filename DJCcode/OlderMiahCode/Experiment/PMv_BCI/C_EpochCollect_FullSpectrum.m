...
%     epochs_beta = [];
    ...
    for mfile = ftemp
        ...
        if (fs == 2400)
            ...
            sig = double(sig); % flx
%             sig = ReferenceCAR(GugerizeMontage(montage.Montage), bads, double(sig)); % flx
...
%             sig = ReferenceCAR(GugerizeMontage(montage.Montage), bads, double(sig));             % flx
        ...
        elseif (fs == 1000)
            % common average re-ref
%             sig = ReferenceCAR(montage.Montage, bads, double(sig)); % flx
            sig = double(sig); % flx
        else
            error('unknown fs');
        end
        ...
        sig = bandpass(sig, 1, 0.4*fs, fs, 4); % flx
        
%         sig = notch(sig, [60; 120], fs, 4); % flx
%         
...
%         fs = fsFinal;
    ...
    % save the rest
    comp = trodesOfInterest{sIdx};
    save(fullfile(META_DIR, [sid '_epochs_fs']), 'montage', 'bads', 'epochs*', '*Dur', 'fs', 'hemi', 'ress', 'src_files', 't', 'tgts', 'cchan');
end