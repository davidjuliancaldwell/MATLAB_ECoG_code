function PlotDotsDirectWithCustomMarkers(subject, trodeLocations, weights, viewSide, clims, markerSize, markerIndicator, colormapName, trodeLabels, plotLabels)
    ...
    possibleMarkers = 'os^vp';
    if length(unique(markerIndicator)) > length(possibleMarkers)
        error('cannot uniquely identify all requested contributors');
    end
...
%             trueColor = (1 - off) * startColor + off * endColor;
        ...
        %     trueColor = (1 - alpha) * minColor + alpha * maxColor;
            plot3(trodeLocations(chan,1),trodeLocations(chan,2),trodeLocations(chan,3),possibleMarkers(markerIndicator(chan)),'MarkerFaceColor',trueColor,'MarkerSize',markerSize,'MarkerEdgeColor','k')%i like red (white would be [1 1 1], etc) dots better
            hold on;
%             diff = counts - chan;
