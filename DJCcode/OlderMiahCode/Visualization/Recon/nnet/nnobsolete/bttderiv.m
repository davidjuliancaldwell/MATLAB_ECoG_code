function out1 = bttderiv(in1,in2,in3,in4,in5,in6,in7)
%BTTDERIV Backpropagation-through-time derivative function.
%
%  <a href="matlab:doc bttderiv">bttderiv</a>('dperf_dwb',net,X,T,Xi,Ai,EW) takes a network, inputs X,
...
%
%  <a href="matlab:doc bttderiv">bttderiv</a>('de_dwb',net,X,T,Xi,Ai,EW) returns the Jacobian, the
%  derivative of each error with respect to the network's weights and biases.
%
%  <a href="matlab:doc bttderiv">bttderiv</a> calculates dynamic derivatives, by applying the derivative
%  chain rule from the last timestep, back through to the first timestep.
...
%    dwb = <a href="matlab:doc bttderiv">bttderiv</a>('dperf_dwb',net,x,t)
%
%  See also DEFAULTDERIV, STATICDERIV, FPDERIV.
...
function info = get_info
  info = nnfcnDerivative(mfilename,'Backpropagation-through-time',...
...
function gWB = calc_gradient(net,data,fcns)
  ...
  fcn = fcns.perform;
  gE(net.outputConnect,:) = nncalc.dperform(...
    net,data.T,data.Y,data.EW,fcn.param);
