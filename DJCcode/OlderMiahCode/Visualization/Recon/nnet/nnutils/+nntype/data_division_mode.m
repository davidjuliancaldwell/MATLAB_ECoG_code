function [out1,out2] = data_division_mode(in1,in2,in3)
%NNTYPE_DATA_DIVISION_MODE Data division mode.
...
function info = get_info
  info = nnfcnType(mfilename,'Data division mode',7.0);
end
...
function err = type_check(x)
  ...
  elseif isempty(nnstring.first_match(lower(x),{'','none','sample','time','sampletime','value'}))
    err = 'VALUE is not ''none'', ''sample'', ''time'', ''sampletime'' or ''all''.';
  else
    err = '';
  end
end
