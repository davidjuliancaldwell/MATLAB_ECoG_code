function [out1,out2] = index_vector_unique(in1,in2,in3)
%NN_INDEX_VECTOR_UNIQUE Index vector unique type.
...
function info = get_info
  info = nnfcnType(mfilename,'Unique Index Row',7.0);
end

function err = type_check(x)
  err = nntype.index_vector('check',x);
  if isempty(err)
    if length(unique(x)) ~= length(x)
      err = 'Indices are not unique.';
    else
      err = '';
    end
  end
