function [out1,out2] = interval(in1,in2,in3)
%NN_INTERVAL Interval type.
...
function info = get_info
  info = nnfcnType(mfilename,'Interval',7.0);
end
...
function err = type_check(x)
  if ~isnumeric(x)
    err = 'Not numeric.';
  elseif ndims(x) ~= 2
    err = 'Not a 2-dimensional matrix.';
  elseif ~all(size(x) == [1 2])
    err = 'Not a 1x2 matrix.';
  elseif x(1) > x(2)
    err = 'First element is greater than second element.';
  else
    err = '';
  end
end
