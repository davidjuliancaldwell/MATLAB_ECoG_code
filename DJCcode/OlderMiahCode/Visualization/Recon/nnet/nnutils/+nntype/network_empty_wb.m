function [out1,out2] = network_empty_wb(in1,in2,in3)
%NN_NETWORK_EMPTY_WB Neural Network, empty, or weight-bias vector type.
...
function err = type_check(x)
  ...
  elseif isnumeric(x)
    if isempty(x)
      err = '';
    elseif max(dims(x)) ~= 1
      err = 'VALUE is not a neural network, empty, or a weight/bias vector.';
    else
      err = '';
    end
  elseif isempty(x)
