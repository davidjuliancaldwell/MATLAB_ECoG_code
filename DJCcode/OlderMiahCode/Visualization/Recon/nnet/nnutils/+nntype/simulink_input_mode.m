function [out1,out2] = simulink_input_mode(in1,in2,in3)
%NNTYPE_SIMULINK_CONNECT_MODE Simulink input mode
...
function err = type_check(x)
  ...
  elseif isempty(nnstring.first_match(lower(x),{'','none','constant','port','workspace'}))
    err = 'VALUE is not '', ''none'', ''constant'', ''port'' or ''workspace''.';
  else
    err = '';
  end
end
