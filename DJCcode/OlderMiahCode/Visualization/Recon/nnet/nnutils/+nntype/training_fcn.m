function [out1,out2] = training_fcn(in1,in2,in3)
%NN_TRAINING_FCN Training function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Training Function',7,...
    8,fullfile('nnet','nntrain'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.training_fcn')
    err = ['VALUE info.type is not nntype.training_fcn.'];
    return;
  end
