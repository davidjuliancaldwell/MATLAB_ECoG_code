...
wins = RvS.windows{1}(:, interestingElectrode,:);
ts = RvS.ts;
restLength = RvS.restLength;
fw = [1:3:200];
fs = pr.fs;
t = ts{1};
h = figure;
...
%vline (0.0225, 'r')
%vline (-0.1558, 'b')
hline (103, 'r--' ,'LineWidth', 1)
hline (22, 'b--', 'LineWidth', 1)
...
%% next plot
hold on
% plot(t, normC(56,:), 'r-') %plots frequency bin at 166 Hz
plot(t, normC(35,:), 'r-') %plots frequency bin at 103 Hz
