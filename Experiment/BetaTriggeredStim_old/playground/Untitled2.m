...
[ecog states] = loadTDTRecording('8adc5c', 'd6', 'BetaTriggeredStim', 67, 49:64);
...
for c = 1:16
	...
	subplot(2,8,c);
    plot(t/ecog_fs, squeeze(mean(res(:, c, poststimidxs),3)),'b', 'linew', 2);    
    hold on;
    plot(t/ecog_fs, squeeze(mean(res(:, c, prestimidxs),3)),'r', 'linew', 2);    
    axis tight
    ylim([-5e-5 5e-5]);
    title(num2str(c));
end
