...
for chan = chans
    ...
    toc;
    %% preprocess eco
    %     presamps = round(1 * efs); % pre time in sec
    %     postsamps = round(2 * efs); % post time in sec
    %
...
%%
save(fullfile(META_DIR, [sid '_LarryStatsRAW.mat']), 't', 'kwinsTotal', 'muCell','stdErrCell','-v7.3');
