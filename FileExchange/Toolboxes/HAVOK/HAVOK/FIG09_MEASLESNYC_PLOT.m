...
FIG09_MEASLESNYC_GEN
% load('./DATA/FIG09_MEASLESNYC.mat');
ModelName = 'MeaslesNYC';
...
[h,hc] = hist(V(:,r)-mean(V(:,r)),[-.1:.02:.1]);%[-.03  -.02 -.015  -.0125 -.01:.0025:.01 .0125  .015 .02  .03]);
[hnormal,hnormalc] = hist(Vtest-mean(Vtest),[-.1:.02:.1])
...
%% PART 8: PREDICT BURSTING OUTBREAK
L = 1:length(V);
inds = V(L,r).^2>1.44e-4;
L = L(inds);
startvals = [];
endvals = [];
start = 1;
...
figure
ax1=subplot(3,1,1)
plot((1:length(V)),V(:,1),'k'), hold on
for k=1:numhits
    plot((startvals(k):endvals(k)),V(startvals(k):endvals(k),1),'r','LineWidth',1.5), hold on
end
for k=1:numhits-1
    plot((endvals(k):startvals(k+1)),V(endvals(k):startvals(k+1),1),'Color',[.25 .25 .25],'LineWidth',1.5), hold on
end
ax2=subplot(3,1,2)
plot((1:length(V)),V(:,r),'k'), hold on
for k=1:numhits
    plot((startvals(k):endvals(k)),V(startvals(k):endvals(k),r),'r','LineWidth',1.5), hold on
end
for k=1:numhits-1
    plot((endvals(k):startvals(k+1)),V(endvals(k):startvals(k+1),r),'Color',[.25 .25 .25],'LineWidth',1.5), hold on
end
ax3=subplot(3,1,3)
plot((startvals(1)),V(startvals(1),r),'r'), hold on
plot((endvals(1)),V(startvals(2),r),'Color',[.25 .25 .25])
for k=1:numhits
    plot((startvals(k):endvals(k)),V(startvals(k):endvals(k),r).^2,'r','LineWidth',1.5), hold on
end
for k=1:numhits-1
    plot((endvals(k):startvals(k+1)),V(endvals(k):startvals(k+1),r).^2,'Color',[.25 .25 .25],'LineWidth',1.5), hold on
end
