...
%     ylim([ymin ymax]);
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontname', 'arial', 'fontsize',16);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontname', 'arial', 'fontsize',16);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontname', 'arial', 'fontsize',16);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontname', 'arial', 'fontsize', 14, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
