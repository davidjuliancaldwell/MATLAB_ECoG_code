function label_add(els,color,msize,plotNames,plotGridLines)
...
if exist('plotNames') && plotNames ~= 0
    ...
    for k=1:length(els(:,1))
        t = text(els(k,1)*1.01,els(k,2)*1.01,els(k,3)*1.01,num2str(k),'FontSize',8,'HorizontalAlignment','center','VerticalAlignment','middle');
        set(t,'clipping','on');
    end
end
...
if exist('plotGridLines') && plotGridLines ~= 0
    ...
    if length(indices) == 2 && size(indices,1)==1
        indices = indices';
    end
