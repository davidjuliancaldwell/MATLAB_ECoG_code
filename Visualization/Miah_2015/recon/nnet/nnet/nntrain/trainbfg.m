...
% $Revision: 1.1.6.18 $ $Date: 2012/04/20 19:14:04 $
  ...
  for epoch=0:param.epochs
    ...
    if isMainWorker
        if param.batch_frag && (Q > 1)
          calcData2 = nn7.y_all(calcNet);
          Ac = calcData2.Ac;
          calcData2 = [];
          Aisize = size(calcData.Ai);
          calcData.Ai = cell(Aisize(1),Aisize(2));
          Aisize = size(calcData.Ai);
          Acsize = size(Ac,2);
          for k1=1:Aisize(1)
            for k2=1:Aisize(2)
              calcData.Ai{k1,k2}(:,2:Q) = Ac{k1,Acsize-Aisize(2)+k2}(:,1:Q-1);
            end
          end
        end
