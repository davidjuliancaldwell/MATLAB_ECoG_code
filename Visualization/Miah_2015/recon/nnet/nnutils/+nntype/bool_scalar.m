function [out1,out2] = bool_scalar(in1,in2,in3)
%NN_BOOL_SCALAR Boolean scalar type.
...
function info = get_info
  info = nnfcnType(mfilename,'Scalar Boolean',7.0);
end

function err = type_check(x)
  if ~(isnumeric(x) || islogical(x))
    err = 'Not numeric or logical.';
  elseif numel(x) ~= 1
    err = 'Not a scalar.';
  elseif ~(isfinite(x) && ((x==0) || (x==1)))
    err = 'Not true or false (or 1 or 0).';
  else
    err = '';
  end
end
