function [out1,out2] = cellrow_delayvec(in1,in2,in3)
%NNTYPE_CELLROW_DELAYVEC Cell array row of delay vectors.
...
function err = type_check(x)
  ...
  err = nntype.cellrow('check',x);
  if ~isempty(err), return, end
  for i=1:length(x)
    err = nntype.delayvec('check',x{i});
    if ~isempty(err)
      err = nnerr.value(err,['VALUE{' num2str(i) '}']);
      break
    end
  end
end
