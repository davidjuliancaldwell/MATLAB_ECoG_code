function [out1,out2] = learning_fcn(in1,in2,in3)
%NN_LEARNING_FCN Weight/Bias Learning function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Learning Function',7,...
    7,fullfile('nnet','nnlearn'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.learning_fcn')
    err = [upper(x) '(''type'') is not nntype.learning_fcn.'];
    return;
  end
