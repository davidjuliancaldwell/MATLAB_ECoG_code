function [out1,out2] = network_fcn(in1,in2,in3)
%NN_NETWORK_FCN Network creation function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Network Function',7,...
    7,fullfile('nnet','nnnetwork'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.network_fcn')
    err = [upper(x) ' info.type is not nntype.network_fcn.'];
    return;
  end
  ...
  net = feval(x);
  if ~isa(net,'network')
    err = [upper(x) ' does not return a neural network.'];
    return;
  end
