function [out1,out2] = network_or_null(in1,in2,in3)
%NN_NETWORK_OR_NULL Neural Network type.
...
function err = type_check(x)
  if ~isempty(x) && ~isa(x,'network')
    err = 'VALUE is not a neural network or empty.';
  elseif ~isempty(x) && (numel(x) > 1)
