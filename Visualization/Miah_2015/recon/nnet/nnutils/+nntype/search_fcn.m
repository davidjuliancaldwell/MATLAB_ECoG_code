function [out1,out2] = search_fcn(in1,in2,in3)
%NN_SEARCH_FCN Search function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Line Search Function',7,...
    7,fullfile('nnet','nnsearch'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.search_fcn')
    err = [x ' info.type is not nntype.search_fcn.'];
    return;
  end
