function [out1,out2] = strict_pos_int_inf_scalar(in1,in2,in3)
%NN_STRICT_POS_INT_INF_SCALAR Strictly positive integer or infinite scalar.
...
function info = get_info
  info = nnfcnType(mfilename,'Strictly Positive Integer or Infinite Scalar',7.0);
end
...
function err = type_check(x)
  ...
  elseif isfinite(x) && (x<=0)
    err = 'Not a strictly positive or infinite value.';
  elseif isfinite(x) && (x ~= round(x))
    err = 'Not an integer.';
  else
    err = '';
  end
