function [out1,out2] = strict_pos_int_scalar(in1,in2,in3)
%NN_STRICT_POS_INT_SCALAR Strictly positive integer or infinite scalar.
...
function info = get_info
  info = nnfcnType(mfilename,'Strictly Positive Integer Scalar',7.0);
end
...
function err = type_check(x)
  ...
  elseif ~isfinite(x)
    err = 'Not finite.';
  elseif x <= 0
    err = 'Not a strictly positive value.';
  elseif x ~= round(x)
    err = 'Not an integer.';
  else
    err = '';
