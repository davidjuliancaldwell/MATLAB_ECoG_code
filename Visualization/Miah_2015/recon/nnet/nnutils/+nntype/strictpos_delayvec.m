function [out1,out2] = strictpos_delayvec(in1,in2,in3)
%NNTYPE_STRICTPOS_DELAYVEC Strictly positive delay vector.
...
function err = type_check(x)
  ...
  err = nntype.delayvec(x);
  if ~isempty(err), return; end
  if any(x == 0)
    err = 'VALUE has a zero element.';
  end
end
