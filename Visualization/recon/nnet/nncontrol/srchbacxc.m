function [a,gX,perfb,retcode1,delta,tol,Ac] = srchbacxc(net,X,Pd,T,Ai,Q,TS,dX,gX,perfa,dperfa,delta,tol,ch_perf,fcns)
...
%  Examples
%
%    Here is a problem consisting of inputs X and targets T that we would
...
% $Revision: 1.6.2.5 $ $Date: 2012/03/27 18:07:39 $
    ...
    switch net
      ...
      otherwise,
        try
          ...
          nnerr.throw(['Unrecognized first argument: ''' net ''''])
        end
    end
    return
  end
...
lambda = 1;
X_temp = X + lambda*dX;
net_temp = setwb(net,X_temp);
  
% CALCULATE PERFORMANCE AT NEW POINT
[perfb,Y,Ac,N,Zb,Zi,Zl] = nn7.perf_all2(net_temp,[],Pd,T,Ai,{1},Q,TS,fcns);
g_flag = 0;
cnt1 = cnt1 + 1;
...
while retcode>3
  ...
  if (perfb <= perfa + alpha*lambda*dperfa)         %CONDITION ALPHA IS SATISFIED
    ...
    if (g_flag == 0)
      gX_temp = -calcgxmodref(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,Y,T,perfb,Q,TS,fcns);
      dperfb = gX_temp'*dX;
    end
...
if perfb <= perfa
   if (g_flag == 0)
      gX = -calcgxmodref(net_temp,X_temp,Pd,Zb,Zi,Zl,N,Ac,Y,T,perfb,Q,TS,fcns);
   else
      gX = gX_temp;
   end
...
function info = get_info
  info = nnfcnSearch(mfilename,'Backtracing One-Dimensional Minimization',7);
end
