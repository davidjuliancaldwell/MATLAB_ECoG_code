function [out1,out2] = adaptive_fcn(in1,in2,in3)
%NN_ADAPTIVE_FCN Adaptive function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Adaptive Function',7,...
    7,fullfile('nnet','nnadapt'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.adaptive_fcn')
    err = [upper(x) '(''type'') is not ' mfilename '.'];
    return;
  end
  ...
  % Adapt
   ...
   net2 = adapt(net,x,t);
   y = net2(x);
   perf2 = perform(net2,t,y);
   if (perf2 >= perf1)
     err = [upper(x) ' did not decrease performance after adaption.'];
     return;
   end
end
