function [out1,out2] = simulink_config_mode(in1,in2,in3)
%NNTYPE_SIMULINK_CONFIGURATION_MODE Simulink configuration mode
...
function info = get_info
  info = nnfcnType(mfilename,'Simulink configuration mode',7.0);
end
...
function err = type_check(x)
  ...
  elseif isempty(nnstring.first_match(lower(x),{'','default','discrete'}))
    err = 'VALUE is not ''default'' or ''discrete''.';
  else
    err = '';
  end
end
