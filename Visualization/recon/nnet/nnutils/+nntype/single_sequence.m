function [out1,out2] = single_sequence(in1,in2,in3)
%NN_SINGLE_SEQUENCE Single signal time series data.
...
function info = get_info
  info = nnfcnType(mfilename,'NN Single Signal Data',7.0);
end

function err = type_check(x)
  if ~nntype.data('isa',x)
    err = 'Not NN data.';
  elseif numsignals(x) ~= 1
    err = 'Not a single matrix or cell array with single row.';
  else
    err = '';
  end
end
