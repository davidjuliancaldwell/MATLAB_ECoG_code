function [out1,out2] = topology_fcn(in1,in2,in3)
%NN_TOPOLOGY_FCN Topology function type.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Topology Function',7,...
    7,fullfile('nnet','nntopology'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.topology_fcn')
    err = ['VALUE info.type is not nntype.topology_fcn.'];
    return;
  end
