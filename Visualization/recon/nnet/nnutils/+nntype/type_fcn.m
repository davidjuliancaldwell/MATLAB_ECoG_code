function [out1,out2] = type_fcn(in1,in2,in3)
%NN_TYPE_FCN Type function.
...
function info = get_info
  info = nnfcnFunctionType(mfilename,'Type',7, ...
    7,fullfile('nnet','nntype'));
end
...
function err = type_check(x)
  ...
  info = feval(x,'info');
  if ~strcmp(info.type,'nntype.type_fcn')
    err = ['VALUE info.type is not ' mfilename '.'];
    return;
  end
